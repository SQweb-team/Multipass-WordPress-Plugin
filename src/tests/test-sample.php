...
class SampleTest extends WP_UnitTestCase {
	...
	function test_basic_sign_in_true() {
		$result = sqweb_sign_in( 'test-wp@sqweb.com', 'test-wp:)' );
	...
	function test_basic_check_token_true() {
		$result = sqweb_check_token( sqweb_sign_in( 'test-wp@sqweb.com', 'test-wp:)' ) );
		$this->assertNotEquals( $result, 0 );
	}

	function test_basic_get_websites() {
		$result = sqw_get_sites( sqweb_sign_in( 'test-wp@sqweb.com', 'test-wp:)' ) );
		$this->assertNotEquals( $result, 0 );
	}
}
