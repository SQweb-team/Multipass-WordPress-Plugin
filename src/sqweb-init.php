...
function sqw_filter_content( $content ) {
	...
	global $wpdb;
	$restrictcutartperc = '<div class="sqw-paywall-button-container"><h5>' . __( 'The rest of this article is for subscriber only', 'sqweb' ) . '</h5><span onclick="sqw.modal_first()">' . __( 'Become a subscriber now with Multipass', 'sqweb' ) . '</span><div><img src="https://www.sqweb.com/img/logo_multipass.svg"></div></div>';
	$restrictartbyday = '<div class="sqw-paywall-button-container"><h5>' . sprintf( _n( 'You have already read %d article for free today', 'You have already read %d articles for free today', get_option( 'artbyday' ), 'sqweb' ), get_option( 'artbyday' ) ) . '</h5><p>' . __( 'Please come back tomorrow', 'sqweb' ) . '</p><p>' . __( 'Or', 'sqweb' ) . '</p><span onclick="sqw.modal_first()">' . __( 'Access immediately all the content with Multipass', 'sqweb' ) . '</span><div><img src="https://www.sqweb.com/img/logo_multipass.svg"></div></div>';
	$restrictdateart = '<div class="sqw-paywall-button-container"><h5>' . __( 'This content is for subscriber only', 'sqweb' ) . '</h5><p>' . sprintf( _n( 'It will be available for free in %d day', 'It will be available for free in %d days', ceil( ( get_post_time( 'U', true ) - ( time() - get_option( 'dateart' ) * 86400 ) ) / 86400 ), 'sqweb' ), ceil( ( get_post_time( 'U', true ) - ( time() - get_option( 'dateart' ) * 86400 ) ) / 86400 ) ) . '</p><span onclick="sqw.modal_first()">' . __( 'Become a subscriber now with Multipass', 'sqweb' ) . '</span><div><img src="https://www.sqweb.com/img/logo_multipass.svg"></div></div>';
